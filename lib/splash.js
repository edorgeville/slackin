...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe, gcaptcha_sitekey }){
  let div = dom('.splash',
    ...
    dom('p.status',
      active
        ? [
          dom('b.active', active), ' utilisateurs connectés et ',
          dom('b.total', total), ' inscrits.'
        ]
        : [dom('b.total', total), ' inscrits jusqu\'à présent.']
    ),
    dom('form id=invite',
      channels && (
        channels.length > 1
      ...
      dom('input.form-item type=email name=email placeholder=vous@votredomaine.com '
        + (!iframe ? 'autofocus' : '')),
      dom('br'),
      ...
      coc && dom('.coc',
        dom('label',
          ...
          'J\'accepte les règles de conduite',
          dom('a', { href: coc, target: '_blank' }, 'Règles de conduite'),
          '.'
        )
      ),
      dom('button.loading', 'Recevoir mon invitation')
    ),
    dom('p.signin',
      'ou ',
      dom(`a href=https://${org}.slack.com target=_top`, 's\'identifier'),
      '.'
    ),
    !iframe && dom('footer',
      'powered by ',
